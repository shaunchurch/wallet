...
human_verification:
  - test: "Load dist/ as unpacked extension in Chrome. Click toolbar icon."
    expected: "Popup opens at ~360x600 showing header with 'vibewallet' + sun/moon toggle, '0.00 ETH' balance area, disabled Send/Receive buttons. No CSP errors in DevTools console."
  ...
  - test: "Open chrome://extensions -> vibewallet -> 'Service worker' link."
    expected: "Console shows '[vibewallet] background service worker started'"
...
|----------|----------|--------|---------|
| `package.json` | Project deps with exact pins, name "vibewallet" | VERIFIED | name: "vibewallet", 5 deps + 13 devDeps, zero ^/~ |
...
| `dist/inpage.js` | Inpage IIFE bundle | VERIFIED | `"use strict";(()=>{console.log("[vibewallet] inpage script loaded");})();` |
...
| `src/features/ui/components/Header.tsx` | Header with branding and theme toggle | VERIFIED | "vibewallet" monospace text + Button with sun/moon SVG calling `useTheme().toggle` |
...
**Expected:** Extension installs with vibewallet icon in toolbar. Clicking icon opens popup at ~360x600 showing: header with "vibewallet" text and sun/moon icon button, centered "0.00 ETH" + "$0.00" + "0x0000...0000" address, Send and Receive buttons at bottom (disabled/grayed).
...
**Test:** chrome://extensions -> vibewallet -> click "Service worker" hyperlink.
**Expected:** DevTools opens showing console with `[vibewallet] background service worker started`.
